    ...
    <nav className="bg-secondary sticky top-0 z-50 border-b border-border">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-3">
          <img src={logo} alt="Sunmasters Window Tinting and Shades" className="h-12 md:h-14 w-auto" width={160} height={40} />
            ...
            <div key={link.label} className="relative group">
              {link.hasDropdown ? (
                <div className="relative">
                  <button
                    className="nav-link text-foreground hover:text-primary text-sm font-medium flex items-center gap-1 py-2"
                    onClick={() => toggleDropdown(link.dropdownType!)}
                  >
                    {link.label}
                    <ChevronDown className={`w-3 h-3 transition-transform ${isDropdownOpen(link.dropdownType!) ? 'rotate-180' : ''}`} />
                  </button>
                  {isDropdownOpen(link.dropdownType!) && (
                    <div className={`absolute top-full left-0 mt-1 bg-secondary border border-border rounded-lg shadow-xl z-50 py-2 min-w-[220px] ${link.dropdownType === "locations" ? "max-h-80 overflow-y-auto" : ""}`}>
                      {getDropdownItems(link.dropdownType!).map((subLink) => (
                        <Link
                          key={subLink.href}
                          to={subLink.href}
                          className="block px-4 py-2 text-sm text-muted-foreground hover:text-primary hover:bg-card transition-colors"
                          onClick={() => { setServicesOpen(false); setLocationsOpen(false); }}
                        >
                          {subLink.label}
                        </Link>
                      ))}
                ...
                <Link to={link.href} className="nav-link text-foreground hover:text-primary text-sm font-medium py-2">
                  {link.label}
                </Link>
              ) : (
                <a href={link.href} className="nav-link text-foreground hover:text-primary text-sm font-medium py-2">
                  {link.label}
                </a>
              )}
            </div>
